...
target_adj = r"""  db\.stock_movements\.push\(movement\);
...
repl_adj = r"""  db.stock_movements.push(movement);
  syncSingleDoc("stock_movements", movement._id, movement);
  if (location_type === "SALES") {
    syncSalesStockLedger(location_id, sku_id, todayStr);
  }
  ...
  try {
    ...
    sqlDb.insert(pgStockMovements).values({
      ...
      officeId: movement.warehouse_id,
    ...
    console.error("Error with Postgres adjustment sync:", err.message);
  }"""

content = re.sub(target_adj, repl_adj, content)
