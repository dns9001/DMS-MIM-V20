...
const targetStr = `        // Deduct stock from SALES inventory (or warehouse fallback if non-sales admin testing)
...
const replacement = `        // ATOMIC POSTGRES DEDUCTION
...
code = code.replace('const processedItems = items.map((it: any, idx: number) => {', 'const processedItems = await Promise.all(items.map(async (it: any, idx: number) => {');
