...
export async function postSaleAtomic(input: {
  ...
  return await sqlDb.transaction(async (tx) => {
    ...
    for (const item of cleanItems) {
      ...
      const lineGross = item.quantity * item.unitPrice, lineSubtotal = Math.max(0, lineGross - item.discount);
      subtotal += lineSubtotal; discountTotal += item.discount;
      normalized.push({ id: id("txi"), transactionId: "", skuId: item.sku_id, productId: sku[0].productId || null, quantity: item.quantity, volume: item.quantity, unitPrice: item.unitPrice, discountAmount: item.discount, subtotal: lineSubtotal, metadata: { source: "ATOMIC_SALE_POST" } });
    }
